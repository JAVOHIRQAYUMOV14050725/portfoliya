...
import { usePublicProjectsQuery } from "../api/projectApi";
...
const LiveDemoIcon = (props) => (
  ...
  >
    <path
      ...
      d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m-5.25 0H21v5.25"
    />
  </svg>
);
...
const Projects = () => {
  ...
  // 🔁 Har bir rasm xatosini track qilish uchun state
  const [imageErrorStates, setImageErrorStates] = useState({});

  // 🌐 Tilda o‘zgarish bo‘lganda qayta ishlansin
  useEffect(() => {
    setImageErrorStates((prev) => ({ ...prev }));
  }, [i18n.language]);

  if (isLoading) {
    return (
      <Section>
        <SectionTitle>{t("projects.loading")}</SectionTitle>
      </Section>
    );
  }
  ...
  if (isError) {
    return (
      <Section>
        <SectionTitle>{t("projects.error")}</SectionTitle>
      </Section>
    );
  }
    ...
    <Section id="projects" className="bg-slate-50 dark:bg-slate-900/50">
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        {projects.map((project, index) => {
          const fallbackText = encodeURIComponent(t("projects.imageNotFound"));

          const handleImageError = () => {
            setImageErrorStates((prev) => ({ ...prev, [project.id]: true }));
          };

          const imageSrc = imageErrorStates[project.id]
            ? `https://placehold.co/600x400/1a202c/ffffff?text=${fallbackText}`
            : project.imageUrl;
